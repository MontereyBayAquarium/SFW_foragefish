...
library(openxlsx)                                                                                             # LOAD PACKAGE TO READ AND WRITE EXCEL FILES, PACKAGE MAY NEED TO BE INSTALLED FIRST
library(readxl)                                                                                               # LOAD PACKAGE TO READ AND WRITE EXCEL FILES, PACKAGE MAY NEED TO BE INSTALLED FIRST
...
B=as.matrix(group.data$B)                                                                                     # CREATE VECTOR FOR BIOMASS (B) VALUES PER GROUP IN T/KM2

QB=as.matrix(group.data$QB)                                                                                   # CREATE VECTOR FOR CONSUMPTION (Q) PER BIOMASS (B) PER GROUP IN T/YEAR
...
{                                                                                                             # OPEN LOOP
  ...
  if (group.names[i]=='Import'|group.names[i]=='Discards'|group.names[i]=='By-catch')                         # CONDITION: IF THE GROUP NAME OF GROUP i IS 'IMPORT', 'BY-CATCH', OR 'DISCARDS' (OUTSIDE (NATURAL) ECOSYSTEM) THEN
  {
...
for (i in 1:n.groups)                                                                                         # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO NUMBER OF GROUPS 
{
  connectance=rbind(connectance,(S2[i]/sum(S)))                                                               # POPULATE 'CONNECTANCE' INDEX FOR GROUP i WITH TOTAL TROPHIC LINKAGES (S2) OF GROUP i DIVIDED BY TOTAL TROPHIC LINKAGES IN FOOD WEB
  
  if (connectance[i]>0.04)                                                                                    # CONDITION: IF 'CONNECTANCE' INDEX FOR GROUP i IS HIGHER THAN 0.04, THEN
  {KEYconnectance=rbind(KEYconnectance,KEYconnectance[i]<-'KEY')}                                             # POPULATE 'KEYCONNECTANCE' FOR GROUP i WITH 'KEY' AS THIS GROUP IS IDENTIFIED AS BEING KEY BY THE CONNECTANCE INDEX
  
  else                                                                                                        # IF 'CONNECTANCE' INDEX FOR GROUP i IS NOT HIGHER THAN 0.04 (Smith et al., 2011), THEN
  {KEYconnectance=rbind(KEYconnectance,KEYconnectance[i]<-'')}                                                # LEAVE 'KEYCONNECTANCE' FOR GROUP i EMPTY AS THIS GROUP IS NOT IDENTIFIED AS BEING KEY BY THE CONNECTANCE INDEX
  
  sumpij<- rbind(sumpij,sum((p[i,])^2))                                                                       # POPULATE 'sumpij' FOR GROUP i WITH VALUE CALCULATED AS THE SUM OF ALL SQUARED DIET PROPORTION GROUP i REPRESENTS FOR EACH GROUP AS PREDATOR
  SURF<- rbind(SURF,sumpij[i]/sum(S))                                                                         # POPULATE 'SURF' INDEX FOR GROUP i AS 'sumpij' OF GROUP i DIVIDED BY TOTAL TROPHIC LINKAGES IN FOOD WEB
  
  if (SURF[i]>0.001)                                                                                          # CONDITION: IF 'SURF' INDEX OF GROUP i IS HIGHER THAN 0.001 (Plaganyi and Essington, 2014), THEN
  {KEYSURF=rbind(KEYSURF,KEYSURF[i]<-'KEY')}                                                                  # POPULATE 'KEYSURF' FOR GROUP i WITH 'KEY' AS THIS GROUP IS IDENTIFIED AS BEING KEY BY THE MASS SURF INDEX
  else                                                                                                        # IF 'SURF' INDEX FOR GROUP i IS NOT HIGHER THAN 0.001, THEN
  {KEYSURF=rbind(KEYSURF,KEYSURF[i]<-'')}                                                                     # LEAVE 'KEYSURF' FOR GROUP i EMPTY AS THIS GROUP IS NOT IDENTIFIED AS BEING KEY BY THE MASS SURF INDEX
  
  esumpij<- rbind(esumpij,sum((pE[i,])^2))                                                                    # POPULATE 'esumpij' FOR GROUP i WITH VALUE CALCULATED AS THE SUM OF ALL SQUARED DIET PROPORTION GROUP i REPRESENTS FOR EACH GROUP AS PREDATOR
  energy<- rbind(energy,esumpij[i]/sum(ES))                                                                   # POPULATE 'energy' SURF INDEX FOR GROUP i AS 'esumpij' OF GROUP i DIVIDED BY TOTAL TROPHIC LINKAGES IN FOOD WEB
  
  if (energy[i]>0.001)                                                                                        # CONDITION: IF 'SURF' INDEX OF GROUP i IS HIGHER THAN 0.001 (Surma et al., 2022), THEN 
  {KEYenergy=rbind(KEYenergy,KEYenergy[i]<-'KEY')}                                                            # POPULATE 'KEYSURF' FOR GROUP i WITH 'KEY' AS THIS GROUP IS IDENTIFIED AS BEING KEY BY THE ENERGY SURF INDEX
  else                                                                                                        # IF 'ENERGY' INDEX FOR GROUP i IS NOT HIGHER THAN 0.001, THEN
  {KEYenergy=rbind(KEYenergy,KEYenergy[i]<-'')}                                                               # LEAVE 'KEYENERGY' FOR GROUP i EMPTY AS THIS GROUP IS NOT IDENTIFIED AS BEING KEY BY THE ENERGY SURF INDEX
  
  if (group.names[i]=='Import'|group.names[i]=='Discards'|group.names[i]=='By-catch')                         # CONDITION: IF GROUP i IS EITHER 'IMPORT', 'DISCARDS' or 'BY-CATCH', THEN 
  {
    KEYconnectance[i]<-''                                                                                     # LEAVE 'KEYCONNECTANCE' EMPTY
    KEYSURF[i]<-''                                                                                            # LEAVE 'KEYSURF' EMPTY
    KEYenergy[i]<-''                                                                                          # LEAVE 'KEYENERGY' EMPTY
  }
}

colnames(connectance)<-'connectance'                                                                          # NAME COLUMN
colnames(KEYconnectance)<-'KEYconnectance'                                                                    # NAME COLUMN
colnames(SURF)<-'SURF mass'                                                                                   # NAME COLUMN
colnames(KEYSURF)<-'KEYSURF mass'                                                                             # NAME COLUMN
colnames(energy)<-'SURF energy'                                                                               # NAME COLUMN
colnames(KEYenergy)<-'KEYSURF energy'                                                                         # NAME COLUMN

indices<-data.frame(group.names,round(connectance,4),KEYconnectance,round(SURF,4),KEYSURF, round(energy,4),KEYenergy)   # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME
worksheetmass<-data.frame(group.names,S,S2,sumpij,round(connectance,4),KEYconnectance,round(SURF,4),KEYSURF,p)          # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME
worksheetenergy<-data.frame(group.names,ES,esumpij,round(energy,4),KEYenergy,pE)                                        # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME
list_of_datasets <- list("Table A - Model Parameters" = group.data, "Table B - Diet Mass" = worksheetmass, "Table C - Diet Energy" = worksheetenergy)# CREATE LIST OF DATA FOR SEPERATE EXCEL DATASHEETS
...
for (i in 1:obs1){                                                                                            # RUN LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF GROUP NAMES
  if(length(which(unique(group.data$agg.group.name[i])==group.data$agg.group.name))>1)                        # IF GROUP NAME i OCCURS MORE THAN ONCE (AS FOR THE NEW AGGREGATED GROUPS) IN THE GROUP.DATA TABLE
  {        
    combine.group.names.all<-append(combine.group.names.all,group.data$agg.group.name[i])                     # EXTRACT THOSE AGGREGATED GROUP NAMES FROM GROUP DATA AND ADD TO LIST (THIS WILL CONTAIN NAME OF AGGREAGETED GROUP MULTIPLE TIMES)
  }
}
...
for (i in 1:obs2){                                                                                            # RUN LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF (NAMES FOR) NEW AGGREGATED GROUPS
  if(length(which(combine.group.names[i]==group.data$agg.group.name))>1)                                    # IF NEW GROUP NAME i OCCURS MORE THAN ONCE (AS FOR THE NEW AGGREGATED GROUPS) IN THE GROUP.DATA TABLE
  {        
    combine.group.list<-append(combine.group.list,tryCatch(list(group.data$group.code[which(combine.group.names[i]==group.data$agg.group.name)])))# LIST THE GROUP CODE OF THOSE GROUPS WITH NEW GROUP NAME i AND ADD THESE CODES PER GROUP NAME TO THE LIST
  }
}
...
for (j in 1:n.groups.2.combine){                                                                              # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF AGGREGATED GROUPS
  aggregated.group.list[[j]]=group.names[combine.group.list[[j]]]                                             # POPULATE LIST WITH NAMES OF AGGREGATED NEW GROUPS
}
...
for(j in 1:n.groups.2.combine){                                                                               # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF AGGREGATED GROUPS
  combine.groups.names<-aggregated.group.list[[j]]                                                            # DETERMINE THE NAMES OF GROUPS TO BE AGGREGATED TO CREATE AGGREGATED GROUP J
  n.combined.groups<-length(combine.groups.names)                                                             # DETERMINE NUMBER OF GROUPS INCLUDED TO CREATED AGGREGATED GROUP J BY COUNTING NUMBER OF GROUP NAMES
  combine.groups<-c()                                                                                         # CREATE EMPTY VECTOR WITH NAMES OF COMBINED GROUPS
  
  for (k in 1:n.combined.groups){                                                                             # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF ORIGINAL GROUPS TO COMBINE TO CREATE AGGREGATED GROUP
    combine.groups[k]<-which(group.names==combine.groups.names[[k]])                                          # LIST THE GROUP CODES OF THE GROUPS NAMES THAT EQUAL THE GROUP NAMES OF THOSE TO BE COMBINED TO CREATE AGGREGATED GROUP K
  }
  
  new.groupname<-combine.group.names[j]                                                                       # ASSIGN NEW GROUPNAME J FROM LIST CREATED IN LINE 256
  
  agg.model=aggregate.groups(combine.groups,new.groupname,C,EC,B,QB,QBE,p,pE,group.names)                     # RUN FUNCTION 'AGGREGATE.GROUPS' PER ABOVE TO CREATE DATA FOR AGGREGATED MODEL
...
for (i in 1:n.groups)                                                                                         # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO NUMBER OF GROUPS 
{
  ...
  esumpij<- rbind(esumpij,sum((pE[i,])^2))                                                                    # POPULATE 'esumpij' FOR GROUP i WITH VALUE CALCULATED AS THE SUM OF ALL SQUARED DIET PROPORTION GROUP i REPRESENTS FOR EACH GROUP AS PREDATOR
  energy<- rbind(energy,esumpij[i]/sum(ES))                                                                   # POPULATE 'energy' INDEX FOR GROUP i AS 'esumpij' OF GROUP i DIVIDED BY TOTAL TROPHIC LINKAGES IN FOOD WEB
  
  if (energy[i]>0.001)                                                                                        # CONDITION: IF 'SURF' INDEX OF GROUP i IS HIGHER THAN 0.001 (Surma et al., 2022), THEN 
  {KEYenergy=rbind(KEYenergy,KEYenergy[i]<-'KEY')}                                                            # POPULATE 'KEYSURF' FOR GROUP i WITH 'KEY' AS THIS GROUP IS IDENTIFIED AS BEING KEY BY THE ENERGY SURF INDEX
  else                                                                                                        # IF 'ENERGY' SURF INDEX FOR GROUP i IS NOT HIGHER THAN 0.001, THEN
  {KEYenergy=rbind(KEYenergy,KEYenergy[i]<-'')}                                                               # LEAVE 'KEYENERGY' FOR GROUP i EMPTY AS THIS GROUP IS NOT IDENTIFIED AS BEING KEY BY THE ENERGY SURF INDEX
  
  if (group.names[i]=='Import'|group.names[i]=='Discards'|group.names[i]=='By-catch')                         # CONDITION: IF GROUP i IS EITHER 'IMPORT','BY-CATCH', OR 'DISCARDS', THEN 
  {
    KEYconnectance[i]<-''                                                                                     # LEAVE 'KEYCONNECTANCE' EMPTY
    KEYSURF[i]<-''                                                                                            # LEAVE 'KEYSURF' EMPTY
    KEYenergy[i]<-''                                                                                          # LEAVE 'KEYENERGY' EMPTY
...
indices.agg<-data.frame(group.names,round(connectance,4),KEYconnectance,round(SURF,4),KEYSURF, round(energy,4),KEYenergy)  # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME 
worksheetmassagg<-data.frame(group.names,S,S2,sumpij,round(connectance,4),KEYconnectance,round(SURF,4),KEYSURF,p)          # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME
worksheetenergyagg<-data.frame(group.names,ES,esumpij,round(energy,4),KEYenergy,pE)                                        # PASTE ALL COLUMNS WITH DATA INTO ONE DATA FRAME
list_of_datasets <- list("Table A - Model Parameters" = group.data, "Table B - Diet Mass" = worksheetmass, "Table C - Diet Energy" = worksheetenergy, "Table D - Diet Mass Agg" = worksheetmassagg, "Table E - Diet Energy Agg" = worksheetenergyagg) # CREATE LIST OF DATA FOR SEPERATE EXCEL DATASHEETS
