...
for (i in 1:n.groups)                                                          # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO NUMBER OF GROUPS 
{
  ...
  if (group.names[i]=='Import'|group.names[i]=='Discards'|
      group.names[i]=='By-catch')                                              # CONDITION: IF GROUP i IS EITHER 'IMPORT', 'DISCARDS' or 'BY-CATCH', THEN 
  {
...
# CREATE FUNTION THAT USES THE ASSIGNED PARAMETERS, SOME ARE BEING GENERATED IN LINES BELOW: COMBINE.GROUPS -> 264 AND NEW.GROUPNAME -> 270
...
for (i in 1:obs1){                                                             # RUN LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF GROUP NAMES
  ...
  {        
    combine.group.names.all<-append(combine.group.names.all,
                                    group.data$agg.group.name[i])              # EXTRACT THOSE AGGREGATED GROUP NAMES FROM GROUP DATA AND ADD TO LIST (THIS WILL CONTAIN NAME OF AGGREAGETED GROUP MULTIPLE TIMES)
  }
}
...
for (i in 1:obs2){                                                             # RUN LOOP WITH NUMBER OF RUNS (i) EQUAL TO # OF (NAMES FOR) NEW AGGREGATED GROUPS
  ...
  {        
    combine.group.list<-append(combine.group.list,
                               tryCatch(list(group.data$group.code[which(combine.group.names[i]==group.data$agg.group.name)]))) # LIST THE GROUP CODE OF THOSE GROUPS WITH NEW GROUP NAME i AND ADD THESE CODES PER GROUP NAME TO THE LIST
  }
}
...
for (i in 1:n.groups)                                                          # RUN A LOOP WITH NUMBER OF RUNS (i) EQUAL TO NUMBER OF GROUPS 
{
  ...
  if (group.names[i]=='Import'|group.names[i]=='Discards'|
      group.names[i]=='By-catch')                                              # CONDITION: IF GROUP i IS EITHER 'IMPORT','BY-CATCH', OR 'DISCARDS', THEN 
  {
    KEYconnectance[i]<-''                                                      # LEAVE 'KEYCONNECTANCE' EMPTY
    KEYSURF[i]<-''                                                             # LEAVE 'KEYSURF' EMPTY
    KEYenergy[i]<-''                                                           # LEAVE 'KEYENERGY' EMPTY
